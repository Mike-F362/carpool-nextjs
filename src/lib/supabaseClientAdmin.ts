...
if (typeof window !== "undefined") {
    throw new Error("supabaseClientAdmin must only be imported on the server " + "(API route, getServerSideProps).");
}

const serviceRoleKey = process.env.SUPABASE_SERVICE_ROLE_KEY;

if (!serviceRoleKey) {
    throw new Error("SUPABASE_SERVICE_ROLE_KEY is not set. See .env.example.local.");
}
