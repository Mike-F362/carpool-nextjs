...
function rng(seed) {                       // mulberry32, damit Laeufe reproduzierbar sind
    return () => {
...
function buildScenario() {
    ...
    const cfg = {
        ...
        stress: {
            ...
            members: [1, 2, 3, 4].map(i => ({id: i, label: 'A' + i, stop: 1, canDrive: true}))
...
function lastDriveMap(history) {
    const m = new Map();
    for (const t of history) for (const id of t.drivers) {
...
function suggestBuckets(history, present, warn) {
    ...
    const pick = (cands, q) => [...cands].sort((a, b) => {
    ...
    if (quotesA.size === 0) {                       // Partial-Match-Fallback aus new_day.tsx
...
function run(name, strategy) {
    const history = [], ledger = new Map(), warn = {fallback: 0, ambiguous: 0};
    for (const day of SCHEDULE) {
        const drivers =
            strategy === 'historisch' ? day.historic :
            strategy === 'bucket' ? suggestBuckets(history, day.present, warn) :
...
for (const m of MEMBERS) {
    ...
    p(`  ${(m.label + ' dabei').padEnd(16)}: ${String(n).padStart(3)} von ${SCHEDULE.length} (${Math.round(100 * n / SCHEDULE.length)} %)`);
}
...
for (const basis of ['fahrer', 'mitfahrer']) {
    ...
    for (const r of results) {
        ...
        for (let leg = 1; leg <= N_LEGS; leg++) {
            for (const m of MEMBERS) {
                ...
                if (!s?.tage) continue;
                rows.push([r.name, m.label, leg, s.soll.toFixed(1), s.ist.toFixed(0),
    ...
    for (const r of results) {
        const st = evaluate(r.history, basis);
        sp.push([r.name, ...Array.from({length: N_LEGS}, (_, i) => {
            const d = MEMBERS.map(m => st.get(`${m.id}|${i + 1}`)).filter(s => s?.tage)
                .map(s => s.ist - s.soll);
...
if (!argv.includes('--no-detail')) {
    ...
    SCHEDULE.forEach((d, i) => rows.push([d.date, d.present.map(lbl).join(','),
        ...results.flatMap(r => r.history[i].drivers.map(lbl))]));
    table(rows);
}
