...
const DIR = argv.includes('--data') ? argv[argv.indexOf('--data') + 1]
...
const TOURS = csv('fahrten.csv').map(l => {
    const m = l.match(/^([^,]*),([^,]*),([^,]*),"?(\[[^\]]*\])"?,([^,]*),([^,]*)$/);
...
const pick = (cands, score, last) => [...cands].sort((a, b) => {
...
for (let i = 0; i < TOURS.length; i++) {
    const hist = TOURS.slice(0, i), cur = TOURS[i];
    ...
    for (const [name, fn] of Object.entries(VERFAHREN)) {
        ...
        const r = res[name];
        if (nA > 1) {                                   // nur echte Entscheidungen zaehlen
            r.n++;
            if (a === cur.drivers[0]) r.a++; else r.missA.push({...cur, pred: a});
        }
        if (nB > 1) {
            r.nb++;
            if (b === cur.drivers[1]) r.b++; else r.missB.push({...cur, pred: b});
        }
        if (fb) r.fb++;
    }
}
...
for (const [von,bis] of [["0000-00-00",SPLIT],[SPLIT,"9999"]]) {
  const sub={};
  for (const name of Object.keys(VERFAHREN)) sub[name]={a:0,n:0,b:0,nb:0};
  for (let i=0;i<TOURS.length;i++){
    const cur=TOURS[i]; if(!(cur.date>=von&&cur.date<bis))continue;
    const hist=TOURS.slice(0,i);
    const nA=cur.present.filter(id=>byId.get(id).stop===1).length;
    const nB=eligible(cur.present,2,cur.drivers[0]).length;
    for(const [name,fn] of Object.entries(VERFAHREN)){
      const {a,b}=fn(hist,cur.present,cur.drivers[0]); const r=sub[name];
      if(nA>1){r.n++; if(a===cur.drivers[0])r.a++;}
      if(nB>1){r.nb++; if(b===cur.drivers[1])r.b++;}
    }
  }
  console.log(`\n### Zeitraum ${von==="0000-00-00"?"bis":"ab"} ${bis===  "9999"?von:bis}`);
  for(const [name,r] of Object.entries(sub))
    console.log("  "+name.padEnd(32)+" Et.1 "+String(r.a).padStart(3)+"/"+String(r.n).padStart(3)+" = "+(r.n?(100*r.a/r.n).toFixed(0):"-").padStart(3)+" %   Et.2 "+String(r.b).padStart(3)+"/"+String(r.nb).padStart(3)+" = "+(r.nb?(100*r.b/r.nb).toFixed(0):"-").padStart(3)+" %");
}
console.log();
...
for (const [name, r] of Object.entries(res)) {
    const pa = (100 * r.a / r.n).toFixed(1), pb = (100 * r.b / r.nb).toFixed(1);
...
for (const m of bk.missA) {
    console.log(`  ${m.date}  anwesend ${m.present.map(lbl).join(',')}  ->  historisch ${lbl(m.drivers[0])}, Algorithmus ${lbl(m.pred)}`);
}
...
for (const m of bk.missB) {
    console.log(`  ${m.date}  anwesend ${m.present.map(lbl).join(',')}  A=${lbl(m.drivers[0])}  ->  historisch ${lbl(m.drivers[1])}, Algorithmus ${lbl(m.pred)}`);
}
