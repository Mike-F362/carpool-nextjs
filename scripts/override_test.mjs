...
const SCHEDULE = csv('fahrten.csv').map(l => {
    const m = l.match(/^([^,]*),([^,]*),([^,]*),"?(\[[^\]]*\])"?,([^,]*),([^,]*)$/);
...
function balanceSuggest(ledger, hist, present, leg, prevDriver) {
    return pick(eligible(present, leg, prevDriver),
        x => ledger.get(`${x}|${leg}`) ?? 0, lastDrive(hist));
}

/** Ein Durchlauf mit Eingriffswahrscheinlichkeit p. */
function runOnce(strategy, p, seed) {
    const rand = rng(seed);
    const hist = [], ledger = new Map();
...
function spread(hist, basis) {
    ...
    return Array.from({length: N_LEGS}, (_, i) => {
        const d = MEMBERS.map(m => st.get(`${m.id}|${i + 1}`)).filter(Boolean)
            .map(s => s.ist - s.soll);
...
for (const [strat, name, basis] of [...VERFAHREN, ['zufall', 'Zufall (Referenz: keinerlei Steuerung)', 'fahrer']]) {
    ...
    for (const p of (strat === 'zufall' ? [1] : PS)) {
        const runs = Array.from({length: p === 0 ? 1 : SEEDS},
            (_, s) => spread(runOnce(strat === 'zufall' ? 'fahrer' : strat, p, s + 1), basis));
...
for (const [strat, name, basis] of VERFAHREN) {
    ...
    console.log(`  ${name.padEnd(34)} ${r.erholt === null
