...
describe('Row Level Security', () => {
    ...
    test('keine Policy erlaubt bedingungslos alles', () => {
        const offen = [...sql.matchAll(/CREATE POLICY\s+"([^"]+)"\s+ON\s+"?public"?\."?(\w+)"?[^;]*?USING\s*\(\s*true\s*\)/gi)]
            .map(m => `${m[2]}: "${m[1]}"`);
    ...
    test('Rollenpruefung nutzt app_metadata, nicht user_metadata', () => {
        ...
        const treffer = [...sql.matchAll(/CREATE POLICY\s+"([^"]+)"[^;]*user_metadata/gi)]
            .map(m => m[1]);
        assert.deepEqual(treffer, [],
            'Policies pruefen die Rolle in user_metadata (client-beschreibbar): '
            + treffer.join(', '));
    });

    test('Schreibrechte auf fahrten liegen nicht bei anon', () => {
        assert.doesNotMatch(sql, /GRANT ALL ON TABLE\s+"?public"?\."?fahrten"?\s+TO\s+"?anon"?/i,
...
describe('Anwendungscode prueft die Rolle serverseitig', () => {
    test('withAdminAuth verlaesst sich nicht auf user_metadata', () => {
        ...
        assert.doesNotMatch(src!, /user_metadata\s*\?\.\s*role/,
...
describe('Secrets und Konfiguration', () => {
    test('der Service-Role-Key traegt kein NEXT_PUBLIC_-Praefix', () => {
        ...
        assert.doesNotMatch(src!, /NEXT_PUBLIC_SUPABASE_SERVICE_ROLE_KEY/,
    ...
    test('die Beispiel-Umgebung nennt alle benoetigten Variablen', () => {
        ...
        for (const v of ['NEXT_PUBLIC_SUPABASE_URL', 'NEXT_PUBLIC_SUPABASE_ANON_KEY',
            ...
            assert.match(env, new RegExp(v),
                `${v} fehlt in der Beispiel-Umgebung - die App startet ohne sie nicht`);
        }
    });
    ...
    test('Abhaengigkeiten sind auf Versionen festgelegt', () => {
        ...
        const lose = Object.entries({...pkg.dependencies, ...pkg.devDependencies})
            .filter(([, v]) => v === 'latest' || v === '*')
            .map(([k]) => k);
        assert.deepEqual(lose, [],
            'nicht festgelegte Versionen machen Builds unreproduzierbar: ' + lose.join(', '));
    });
...
describe('Browser-Kompatibilitaet', () => {
    test('kein Set.difference / Set.intersection im Anwendungscode', () => {
        ...
        const walk = (dir: string) => {
            for (const e of fs.readdirSync(dir, {withFileTypes: true})) {
                ...
                if (e.isDirectory()) walk(p);
                    ...
                    if (/\.(difference|intersection|symmetricDifference)\s*\(/.test(s)
        ...
        walk(path.join(ROOT, 'src'));
        assert.deepEqual(treffer, [],
            'sehr neue ES-Methoden ohne breite Browserunterstuetzung: ' + treffer.join(', '));
    });
});
