...
import {
    buildLedger, eligibleDrivers, evaluateFairness, lastDriveDates, legCount,
    postTour, spreadOnLeg, suggestDrivers, type Ledger, type Options, type Tour,
...
function simulate(members: ReturnType<typeof makeMembers>, schedule: {date: string; present: number[]}[],
                  opts: Options, overrideP = 0, seed = 1): Tour[] {
    const r = rng(seed);
    const history: Tour[] = [];
    ...
    for (const day of schedule) {
        ...
        for (let leg = 1; leg <= legCount(members); leg++) {
            ...
            let d = leg === 1 ? suggested[0]! : cands.includes(suggested[leg - 1] ?? -1)
                ? suggested[leg - 1]!
                : cands[0];
    ...
    describe(`Invarianten, Massstab ${basis}`, () => {
        ...
        test('gilt auch bei mehr Fahrern und drei Stopps', () => {
            for (let seed = 1; seed <= 20; seed++) {
                const members = [...makeMembers(4, 2),
                    {id: 20, label: 'C1', stop: 3, canDrive: true}];
        ...
        test('die Spreizung waechst nicht mit der Laufzeit', () => {
            ...
            for (let seed = 1; seed <= 8; seed++) {
                ...
                for (let leg = 1; leg <= legCount(members); leg++) {
                    ...
                    assert.ok(s < 2.0,
                        `seed ${seed}, Etappe ${leg}: nach 960 Fahrten Spreizung ${s.toFixed(2)}`);
                }
            }
        });
        ...
        test('Summe der Salden je Etappe bleibt null', () => {
            ...
            for (let leg = 1; leg <= legCount(members); leg++) {
                const sum = [...ledger].filter(([k]) => k.endsWith(`|${leg}`))
                    .reduce((a, [, v]) => a + v, 0);
...
describe('Massstab mitfahrer entlastet den Fahrer am Zwischenstopp', () => {
    test('B-Fahrer faehrt Etappe 2 seltener als unter Massstab fahrberechtigte', () => {
        const members = makeMembers(3, 1);
        const sched = randomSchedule(members, 300, 0.15, 99)
            .filter(d => d.present.includes(4));          // nur Tage mit B-Fahrer
        ...
        const a = anteil('fahrberechtigte'), b = anteil('mitfahrer');
