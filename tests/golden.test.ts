...
const FALK = members.find(m => m.stop === 2)!;
...
describe('Bilanz der echten Historie', () => {
    ...
    test('Etappe 1: die drei Startpunktfahrer liegen bei 43 / 46 / 54', () => {
        ...
        assert.equal(n('Mike'), 43);
        assert.equal(n('Martin'), 46);
        assert.equal(n('Falko'), 54);
    });
    ...
    test('sein Anteil an Etappe 2 liegt unabhaengig von der Gruppengroesse bei rund 50 %', () => {
        for (const n of [3, 4]) {
            const g = tours.filter(t => t.present.includes(FALK.id) && t.present.length === n);
            const anteil = g.filter(t => t.drivers[1] === FALK.id).length / g.length;
